...
fn ui(frame: &mut Frame, app: &App) {
    ...
    render_instances(frame, content[0], app);
    render_right_bar(frame, content[1], app);
}

fn render_instances(frame: &mut Frame, area: Rect, app: &App) {
    ...
    for (i, (name, playtime)) in app.instances.iter().enumerate() {
        ...
        rows.push(Row::new(vec![Cell::from(format!("{} {}", marker, name)), Cell::from(*playtime)]).style(style));
    }
    ...
    frame.render_widget(
        table.block(Block::default().borders(Borders::ALL).title("Instances")),
        area,
    );
}
